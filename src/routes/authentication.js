...
router.post('/signup', async (req, res) => {
    ...
    const user2 = await userSchema.findOne({ correo: user.correo });
    //validando si no se encuentra
    if (user2) return res.status(400).json({ error: "Usuario usurario ya existe" });
    ...
    res.json({ auth: true, message: "Usuario creado con éxito" });
});
